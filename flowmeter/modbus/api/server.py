...
class FlowMeterClients:
    ...
    def get_dtu_no(self, ip):
        return self.ip_to_dtu_map[ip] if ip in self.ip_to_dtu_map.keys() else None
...
def run_server(port=8081):
    ...
    # 创建定时任务，定时循环调用
    task1 = task.LoopingCall(exec_remote_opr)
    # 开启定时任务，并指定定时任务的时间间隔
    task1.start(conf_configure_api.get_unexecuted_opr_check_time())
...
def is_dtu_online(dtu_no):
    ...
    :return:
    """
    return dtu_no in clients.get_online_dtu_no_list()
